...
class LASMConvssw(nn.Module):
    def __init__(self, in_channel, out_channel, weight_num,in_point_num,  connection_info, b_Perpt_bias = True,  residual_rate = 0.0): #layer_info_list= [(point_num, feature_dim)]
        ...
        neighbor_num_list = torch.from_numpy(connection_info[:,0].astype(np.float32)).float() #out_point_num*1
        self.register_buffer("neighbor_num_list", neighbor_num_list)

        neighbor_id_dist_list = connection_info[:, 1:] #out_point_num*(max_neighbor_num*2)
        neighbor_id_list = neighbor_id_dist_list.reshape((out_point_num, -1,2))[:,:,0] #out_point_num*max_neighbor_num

        neighbor_id_list = torch.from_numpy(neighbor_id_list).long()
        self.register_buffer("neighbor_id_list", neighbor_id_list)

        max_neighbor_num = neighbor_id_list.shape[1]
        self.max_neighbor_num = max_neighbor_num

        avg_neighbor_num= round(neighbor_num_list.mean().item())
    ...
    def forward(self, in_pc, raw_w_weights, is_final_layer=False, b_max_pool=False):
        ...
        neighbor_id_list = self.neighbor_id_list


        pc_mask  = torch.ones(in_pn+1).float().to(in_pc.device)
        pc_mask[in_pn] = 0
        neighbor_mask_list = index_selection_nd(pc_mask,neighbor_id_list,0).contiguous()#out_pn*max_neighbor_num neighbor is 1 otherwise 0

        raw_weights = self.weights
        bias = self.bias

        w_weights = raw_w_weights*(neighbor_mask_list.view(out_pn, max_neighbor_num, 1)) #out_pn*max_neighbor_num*weight_num

        # print('in_pc:', in_pc.size())
        # print('other:', torch.zeros(batch, 1, in_channel).size())
        # print("in_pc.shape: ", in_pc.shape)
        in_pc_pad = torch.cat((in_pc, torch.zeros(batch, 1, in_channel).float().to(in_pc.device)), 1) #batch (in_pn+1) in_channel
        in_neighbors = index_selection_nd(in_pc_pad,neighbor_id_list, 1)
        ...
        out_pc = None #batch*out_pn*out_channel
        if b_max_pool:
        ...
        if(in_pn == out_pn):
            ...
            #p_neighbors = torch.sigmoid(p_neighbors_raw) * neighbor_mask_list
            p_neighbors = torch.abs(p_neighbors_raw)  * neighbor_mask_list
    ...
    def forward2(self, in_pc, raw_w_weights, is_final_layer=False, b_max_pool = False): #layer_info
        ...
        pc_mask[in_pn]=0
        neighbor_mask_list = pc_mask[neighbor_id_list].contiguous() #out_pn*max_neighbor_num neighbor is 1 otherwise 0


        raw_weights = self.weights
        bias = self.bias

        w_weights = raw_w_weights*neighbor_mask_list.view(out_pn, max_neighbor_num, 1).repeat(1,1,weight_num) #out_pn*max_neighbor_num*weight_num
        ...
        in_neighbors = in_pc_pad[:, neighbor_id_list] #batch*out_pn*max_neighbor_num*in_channel
        ...
        if(in_pn == out_pn):
            ...
            p_neighbors_raw = self.p_neighbors
            in_neighbors = in_pc_pad[:,neighbor_id_list] #batch*out_pn*max_neighbor_num*out_channel
...
class MCEnc(nn.Module):
    def __init__(self,structure, channel_list,weight_num): #layer_info_list= [(point_num, feature_dim)]
        ...
        self.layer_num = len(structure.connection_info_lists)
        ...
        for l in np.arange(0,self.layer_num):
            in_channel = self.channel_list[l]
            out_channel = self.channel_list[l+1]
            connection_info  = structure.connection_info_lists[l]
            print('in:',in_channel, 'out:',out_channel)

            in_point_num = structure.point_num_list[l]

            self.layer_list.append(LASMConvssw(in_channel, out_channel, weight_num,in_point_num,  connection_info, b_Perpt_bias, self.residual_rate))
        ...
        self.out_number_points = structure.point_num_list[self.layer_num]
        self.out_number_channels = out_channel

        print(self.layer_num, self.out_number_points, self.out_number_channels)


    def forward_till_layer_n(self,in_pc,vcoeffs, layer_n):
        out_pc = in_pc.clone()
        for i in range(layer_n):
            out_pc = self.layer_list[i](out_pc,vcoeffs.vcoeffs_list[i], is_final_layer = False, b_max_pool = self.b_max_pool)
        return out_pc

    def forward(self, in_pc, vcoeffs):
        tmpcode = self.forward_till_layer_n(in_pc, vcoeffs, self.layer_num-1)
        mu = self.layer_list[self.layer_num-1](tmpcode,vcoeffs.vcoeffs_list[self.layer_num-1], is_final_layer = True, b_max_pool = self.b_max_pool) * 0.1
    ...
    def forward_from_layer_n(self, in_pc,vcoeffs, layer_n):
        ...
        for i in range(layer_n, self.layer_num):
            if(i < (self.layer_num-1)):
                ...
                out_pc = self.layer_list[i](out_pc,vcoeffs.vcoeffs_list[i], is_final_layer = True, b_max_pool = self.b_max_pool)

        return out_pc

class MCStructure(nn.Module):
    def __init__(self, param, in_points_number, weight_num, bDec=True, b_perpoint_bias=True): #layer_info_list= [(point_num, feature_dim)]
        super(MCStructure, self).__init__()

        self.point_num = in_points_number
        ...
        if bDec:
            self.connection_layer_file_name_list = param.connection_layer_file_name_list_dec
        else:
            self.connection_layer_file_name_list = param.connection_layer_file_name_list_enc
        self.layer_num = len(self.connection_layer_file_name_list)

        self.point_num_list = []
        self.point_num_list += [in_points_number]
        self.connection_info_lists = []
        for l in np.arange(0,self.layer_num):
            print ("##Layer",self.connection_layer_file_name_list[l])

            connection_info  = np.load(self.connection_layer_file_name_list[l])
            out_point_num = connection_info.shape[0]
            self.connection_info_lists += [connection_info]
            self.point_num_list += [out_point_num]
...
class MCVcoeffs(nn.Module):
    def __init__(self, structure, weight_num): #layer_info_list= [(point_num, feature_dim)]
        super(MCVcoeffs, self).__init__()


        self.layer_num = len(structure.connection_layer_file_name_list)
...
class MCLoss(nn.Module):
    def __init__(self,param): #layer_info_list= [(point_num, feature_dim)]
        super(MCLoss, self).__init__()

        self.register_buffer('initial_neighbor_id_list', torch.LongTensor(param.neighbor_id_list))
        self.register_buffer('initial_neighbor_num_list', torch.FloatTensor(param.neighbor_num_list))
        self.initial_max_neighbor_num = self.initial_neighbor_id_list.shape[1]
    ...
    def compute_laplace_loss_l1(self, gt_pc_raw, predict_pc_raw, weights=[]):
        ...
        gt_pc_laplace = gt_pc[:, self.initial_neighbor_id_list[:,0]]   ## batch*point_num*3 the first point is itself
        gt_pc_laplace = gt_pc_laplace*self.initial_neighbor_num_list.view(1, point_num, 1).repeat(batch, 1,3)

        for n in range(1, self.initial_max_neighbor_num):
            neighbor = gt_pc[:,self.initial_neighbor_id_list[:,n]]
            gt_pc_laplace -= neighbor


        predict_pc_laplace = predict_pc[:, self.initial_neighbor_id_list[:,0]]   ## batch*point_num*3 the first point is itself
        predict_pc_laplace = predict_pc_laplace*self.initial_neighbor_num_list.view(1, point_num, 1).repeat(batch, 1,3)

        for n in range(1, self.initial_max_neighbor_num):
            neighbor = predict_pc[:,self.initial_neighbor_id_list[:,n]]
